...
(function() {
  ...
  SgfAniRender = (function() {
    ...
    SgfAniRender.prototype.load = function(wuid) {
      var amfLen, ba, canvasHeight, canvasWidth, err, fileSize, height, i, left, signature, top, url, width, yScroll, _i, _ref;
    ...
    SgfAniRender.prototype.setPlayScript = function(script) {
      var cmd, currentAssetFrame, i, soundWuids, _i, _j, _ref, _ref1;
      if (this.containsError) {
        return this;
      }
      console.log("[sgf-ani-render::setPlayScript] script:" + script);
      this.frames = [0];
      this.frameIdToSoundTrigger = {};
      script = String(script || "").trim();
      if (script === "") {
        for (i = _i = 1, _ref = this.assetFrameNum; _i < _ref; i = _i += 1) {
          this.frames.push(i);
        }
      } else {
        soundWuids = [];
        script = script.replace(/[a-z0-9A-Z]+/g, function(wuid) {
          var url;
        ...
        currentAssetFrame = 0;
        for (i = _j = 0, _ref1 = script.length; _j < _ref1; i = _j += 1) {
          cmd = script.charAt(i);
          if (cmd === "^") {
            this.frameIdToSoundTrigger[currentAssetFrame] = soundWuids.shift();
            continue;
          }
          if (cmd === "+") {
            ++currentAssetFrame;
          }
          if (currentAssetFrame >= this.assetFrameNum) {
            currentAssetFrame = this.assetFrameNum - 1;
          }
          this.frames.push(currentAssetFrame);
        }
      }
      return this;
    };

    SgfAniRender.prototype.toggleRegAid = function() {
      if (this.containsError) {
        return this;
      }
    ...
    SgfAniRender.prototype.goto = function(num) {
      var assetFrame, assetLeft, assetRect, assetTop, framesLength, originalRect, rectLeft, rectTop, soundWuid;
      if (this.containsError) {
        return this;
      }
      ...
      if (num < 0) {
        num = framesLength + (num % framesLength);
      }
      this.currentFrame = num;
      if ((soundWuid = this.frameIdToSoundTrigger[num]) != null) {
        new Audio(SgfAniRender.makeURLFromWuid(soundWuid)).play();
      }
      assetFrame = this.frames[num];
      assetRect = this.assetRects[assetFrame];
      originalRect = this.originalRects[assetFrame];
    ...
    SgfAniRender.prototype.setFps = function(val) {
      ...
      console.log("[sgf-ani-render::setFps] fps:" + this.fps + ", spf:" + this.spf);
      return this;
    };
    ...
    SgfAniRender.prototype.setTitle = function(msg) {
      if (!this.containsError) {
        this.displayLabel(msg);
      }
      return this;
    };
