...
(function() {
  ...
  SgfAniRender = (function() {
    ...
    function SgfAniRender(parentElement) {
      if (!((parentElement != null) && (this.url != null))) {
        throw "[sgf-ani-render::constructor] bad arguments, parentElement:" + parentElement + ", @url:={@url}";
        return;
      }
      this.paper = Raphael(parentElement, SgfAniRender.MIN_CANVAS_SIZE, SgfAniRender.MIN_CANVAS_SIZE);
    }

    SgfAniRender.prototype.load = function(url, config) {
      var amfLen, ba, err, fileSize, height, i, left, signature, top, width, yScroll, _i, _ref,
        _this = this;
      this.url = url;
      if (config == null) {
        config = SgfAniRender.EMPTY_OBJECT;
      }
      config.fixedWidth = parseInt(config.fixedWidth, 10) || 0;
      if (config.fixedWidth < SgfAniRender.MIN_CANVAS_SIZE) {
        config.fixedWidth = 0;
      }
      if (config.fixedWidth > SgfAniRender.MAX_CANVAS_SIZE) {
        config.fixedWidth = SgfAniRender.MAX_CANVAS_SIZE;
      }
      config.fixedHeight = parseInt(config.fixedHeight, 10) || 0;
      if (config.fixedHeight < SgfAniRender.MIN_CANVAS_SIZE) {
        config.fixedHeight = 0;
      }
      if (config.fixedHeight > SgfAniRender.MAX_CANVAS_SIZE) {
        config.fixedHeight = SgfAniRender.MAX_CANVAS_SIZE;
      }
      this.canvasWidth = config.fixedWidth || SgfAniRender.MIN_CANVAS_SIZE;
      this.canvasHeight = config.fixedHeight || SgfAniRender.MIN_CANVAS_SIZE;
      ...
      if (config.title != null) {
        this.paper.text(10, 15, String(config.title)).attr({
          "font-family": "arial",
          "font-size": "14",
          "text-anchor": "start",
          "fill": "#999"
