...
(function() {
  ...
  SgfAniRender = (function() {
    ...
    SgfAniRender.parsePlayScript = function(script, assetFrameCount) {
      var cmd, currentAssetFrame, i, result, _i, _ref;
      assetFrameCount = (assetFrameCount || SgfAniRender.MAX_ASSET_FRAME_COUNT) % (SgfAniRender.MAX_ASSET_FRAME_COUNT + 1);
      if (assetFrameCount < 0) {
        assetFrameCount = 0;
      }
      script = String(script || "").trim().replace(/[^\-\+]/g, '');
      result = [0];
      currentAssetFrame = 0;
      for (i = _i = 0, _ref = script.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        cmd = script.charAt(i);
        if (cmd === "+") {
          ++currentAssetFrame;
        }
        if (currentAssetFrame >= assetFrameCount) {
          currentAssetFrame = assetFrameCount;
        }
    ...
    SgfAniRender.prototype.load = function(wuid, title, fps, playScript) {
    ...
    SgfAniRender.prototype.setPlayScript = function(script) {
      ...
      if (script === "") {
        this.frames = [0];
        for (i = _i = 1, _ref = this.assetFrameNum; 1 <= _ref ? _i < _ref : _i > _ref; i = 1 <= _ref ? ++_i : --_i) {
          this.frames.push(i);
        }
      } else {
        this.frames = SgfAniRender.parsePlayScript(script, this.assetFrameNum);
      }
    };
