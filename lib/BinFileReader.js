...
function BinFileReader(fileURL){
    ...
    /**
	 * 这是一个 BIG_ENDIAN （高位在左）的读取INT的方法
	 *
	 */
	this.readInt = function(iFrom)
	{
		// return this.readNumberBigEndian(4);

		iFrom = iFrom || filePointer;
		this.movePointerTo(iFrom + 4);

		var result = 0;

		for(var i=0; i<4; i++){
			result = result * 256 + this.readByteAt(i+iFrom);
		}
	 ...
	 *
	 */
	this.readShort = function(iFrom)
	{
		iFrom = iFrom || filePointer;
		this.movePointerTo(iFrom + 2);
    ...
    /**
	 * 这是一个 BIG_ENDIAN （高位在右）的读方法
	 *
	 */
	 /* // FIXME: 这个 BIG_ENDIAN 的读取方法工作不正常 by ty
	this.readNumberBigEndian = function(iNumBytes, iFrom){
		...
		var result = 0;
		for(var i=iFrom ; i<iFrom + iNumBytes; i++){
			result = result * 256 + this.readByteAt(i-1);
		}
